...
def model(X, w, w2, w3, w4, w_o, p_keep_conv, p_keep_hidden):
    ...
    l1 = tf.nn.dropout(l1,keep_prob=p_keep_conv)
    ...
    l2 = tf.nn.dropout(l2, keep_prob=p_keep_conv)
    ...
    l3 = tf.reshape(l3, [-1, w4.get_shape().as_list()[0]])                              #l3 = shape(?, 2048)
    l3 = tf.nn.dropout(l3, keep_prob=p_keep_conv)

    l4a = tf.nn.relu(tf.matmul(l3, w4))
    l4 = tf.nn.dropout(l4a, keep_prob=p_keep_hidden)
...
with tf.Session() as sess:
    ...
    for i in range(100):
        ...
        print(i, np.mean(np.argmax(teY[test_indices], axis=1) == sess.run(predict_op, feed_dict={X: teX[test_indices], p_keep_conv:1, p_keep_hidden: 1})))
